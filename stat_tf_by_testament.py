...
fout0=open(file_path0,'w') #打开文件
fout1=open(file_path1,'w') #打开文件
fout2=open(file_path2,'w') #打开文件
...
#将3个字典分别排序,并写入文件
sorted_dict=sorted(keyword2num0.iteritems(), key=lambda d:d[1], reverse = True ) #d[0]为key,d[1]为value,返回一个元组列表
for keyword,number in sorted_dict:
  tmp_line=','.join([keyword,str(number)])
  fout0.write(tmp_line.encode('utf-8')+'\n')
fout0.close() #关闭文件

sorted_dict=sorted(keyword2num1.iteritems(), key=lambda d:d[1], reverse = True ) #d[0]为key,d[1]为value,返回一个元组列表
for keyword,number in sorted_dict:
  tmp_line=','.join([keyword,str(number)])
  fout1.write(tmp_line.encode('utf-8')+'\n')
fout1.close() #关闭文件

sorted_dict=sorted(keyword2num2.iteritems(), key=lambda d:d[1], reverse = True ) #d[0]为key,d[1]为value,返回一个元组列表
for keyword,number in sorted_dict:
  tmp_line=','.join([keyword,str(number)])
  fout2.write(tmp_line.encode('utf-8')+'\n')
fout2.close() #关闭文件
#####################################################
